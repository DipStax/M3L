...
namespace m3l
{
    M3L_API std::vector<std::array<Vertex, 3>> polyTri(const Vertex *_vtx, size_t _size);
    M3L_API Point2<uint32_t> triRange(const Vertex *_vtx, int32_t _line);
    M3L_API Point2<uint32_t> triRange(const Vertex3D* _vtx, int32_t _line);

    namespace priv
    {
        M3L_API bool isEarTip(const CircleList<Vertex> &_cl, int32_t _start);
